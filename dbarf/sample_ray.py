...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1, sim3=None):
        super().__init__()
    ...
    def sample_random_pixel(self, N_rand, sample_mode, center_ratio=0.8, area=[48, 64]):
        if sample_mode == 'center':
            ...
            h, w = area[0], area[1]
            border_H = rng.randint(low=0, high=self.H-h)
            border_W = rng.randint(low=0, high=self.W-w)

            # pixel coordinates
            u, v = np.meshgrid(np.arange(border_H, border_H+h),
                               np.arange(border_W, border_W+w))
            u = u.reshape(-1)
            v = v.reshape(-1)

            select_inds = np.array([i for i in range(u.shape[0])])
            select_inds = v[select_inds] + self.W * u[select_inds]
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8, area=[48, 64]):
        '''
        :param N_rand: number of rays to be casted
        :return:
        '''

        select_inds = self.sample_random_pixel(N_rand, sample_mode, center_ratio, area)

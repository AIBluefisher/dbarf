...
def sample_along_camera_ray(ray_o, ray_d, depth_range,
    ...
    if inv_depth_prior is not None:
        # We sample depth around the depth prior.
        depth_interval = 1.0        # TODO(chenyu): tune this parameter.
        near_depth = torch.clamp(1. / inv_depth_prior - depth_interval, near_depth_value, far_depth_value)
        far_depth = torch.clamp(1.0 / inv_depth_prior + depth_interval, near_depth_value, far_depth_value)

        start = 1. / near_depth     # [N_rays,]
        step = (1. / far_depth - start) / (N_samples-1)
        inv_z_vals = torch.stack([start+i*step for i in range(N_samples)], dim=1)  # [N_rays, N_samples]
        z_vals = (1. / inv_z_vals).squeeze(-1)
...
def render_rays(ray_batch,
    ...
    pts, z_vals = sample_along_camera_ray(ray_o=ray_batch['ray_o'],
                                          ...
                                          inv_depth_prior=inv_depth_prior)
    ...
    rgb_feat, ray_diff, mask = projector.compute(pts, ray_batch['camera'],
                                                 ...
                                                 rel_poses=rel_poses)  # [N_rays, N_samples, N_views, x]

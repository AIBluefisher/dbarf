...
class BasicUpdateBlockRelPose(nn.Module):
    def __init__(self, hidden_dim=128, cost_dim=256, context_dim=64):
        super(BasicUpdateBlockRelPose, self).__init__()
        self.encoder = ProjectionInputPose(cost_dim=cost_dim, hidden_dim=hidden_dim, out_chs=hidden_dim)
        self.pose_gru = SepConvGRU(hidden_dim=hidden_dim, input_dim=self.encoder.out_chs+context_dim)
        self.pose_head = PoseHead(hidden_dim, hidden_dim=hidden_dim)
    ...
    def forward(self, net, cost_func, pose, inp, seq_len=4):
        pose_list = []
        delta_pose = torch.zeros(6, dtype=torch.float, device=pose.device)
        pose = pose.unsqueeze(0)

        for i in range(seq_len):
            # pose = pose.detach()
            input_features = self.encoder(pose, cost_func(rel_pose=pose)) # [1, c, h, w]
            inp_i = torch.cat([inp, input_features], dim=1)

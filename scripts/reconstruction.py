...
def run_reconstruction(sfm_dir, database_path, image_dir, verbose=False):
    ...
    for filename in ['images.bin', 'cameras.bin', 'points3D.bin']:
        ...
        shutil.move(
            str(models_path / str(largest_index) / filename), str(models_path))
    return reconstructions[largest_index]


def main(database, output_dir, image_dir, pairs, features, matches,
    ...
    reconstruction = run_reconstruction(output_dir, database, image_dir, verbose)
